...
export const generateDescription = async (attractions: string): Promise<GenerateContentResult | null> => {
  const prompt = "Now, forget the previous instructions and examples. We are starting a new task. You are a travel guide tasked with creating concise, informative, and engaging one-sentence annotation for the tourist attractions provided in the attached json list."
...
export const generateTextFromWiki = async (wikipedia_page: string, sequence: number, is_last: boolean): Promise<Content | null> => {
  ...
  const prompt = "Your task is to act as an experienced tour guide."
    ...
    +"Your goal is to then create a narrative for audio guide, approximately 500-600 words in length, that describes this tourist attraction as if you were giving a guided tour to a real person. The tone and style should reflect a natural, engaging, and informative conversation between a tour guide and a tourist."
    ...
    +"* Avoid general welcoming words."
    +`* Starting with the name of the attraction and short introduction to it. If required can mention that it's our ${position} point of the excursion.`
